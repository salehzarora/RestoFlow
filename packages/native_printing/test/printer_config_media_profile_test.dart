...
void main() {
  ...
  group('NetworkPrinterConfig', () {
    ...
    test('copyWith changes ONLY the profile, preserving host/port/name', () {
      const base = NetworkPrinterConfig(
        host: '10.0.0.5',
        port: 9101,
        name: 'Front',
      );
  ...
  group('BluetoothPrinterConfig', () {
    test(
      'a fixed-label profile round-trips; legacy absent => continuous80',
      () {
        ...
        final legacy = BluetoothPrinterConfig.fromJson({
          'address': 'DC:0D:30:AA:BB:CC',
        });
    ...
    test(
      'copyWith carries the profile with the whole config (copy action)',
      () {
