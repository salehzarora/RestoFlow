...
void main() {
  group('sortByMenuPrintOrder', () {
    test('orders by the ascending key tuple (category -> item -> position)', () {
      ...
      expect([for (final r in sorted) r.name], const [
        'Burger B',
        'Burger A',
        'Fries',
        'Fanta',
    ...
    test('is STABLE — equal tuples (incl. all-zero legacy) keep input order', () {
      final input = <({String name, int cat, int item, int pos})>[
