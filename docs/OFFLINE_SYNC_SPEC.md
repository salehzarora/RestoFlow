...
The sync operation lifecycle enumeration is a **PROPOSED state enumeration (DECISION D-018; approved into the frozen M0A baseline (RF-004) — RF-001 §8 directs us to evaluate, not assume final)** and its transitions are owned by [STATE_MACHINES.md](STATE_MACHINES.md). This document references it; it does not redefine it.
...
| **Order lifecycle state** (`orders.status`, `order_items.status`) | **Domain-rule merge**, governed by the PROPOSED state machine (**DECISION D-018**, STATE_MACHINES.md; approved into the frozen M0A baseline (RF-004)). Only legal transitions apply; an illegal concurrent transition is `rejected`. | If device A submits and device B tries to add an item to a now-`submitted` order, B's add is evaluated against the new state, not blindly overwritten. |
...
| **Kitchen ticket / station item state** | **Domain-rule merge** per the PROPOSED state machine (approved into the frozen M0A baseline (RF-004)). A `bump` already applied makes a second concurrent `bump` a no-op (idempotent), not a conflict. | KDS bumps must be idempotent and ordered (§5). |
...
6. **Device pairing lifecycle** (`code_issued → pending → paired → active → suspended → revoked`, PROPOSED by **DECISION D-018**; approved into the frozen M0A baseline (RF-004)) governs device state; a `suspended`/`revoked` device's operations are rejected on ingest.
