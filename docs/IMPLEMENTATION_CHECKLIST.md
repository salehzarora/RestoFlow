...
| `STOREFRONT-READ-001` | First real Storefront read: server-rendered live menu path, `public.storefront_menu`, profile / media schema, browse-only live mode | Implemented **locally** on `feat/STOREFRONT-READ-001-live-menu-read` from the sealed integration plan (`worktrees/output/storefront-read-001-plan-20260923T152536Z/`, outside git); **PROPOSED DECISION D-039**; D-037 ratified with one narrow amendment (owner decision D1). Migration `20260923120000` is **not** applied to hosted; the implementation packet makes no push, PR, build or provider change. Follow-ups, each its own Work ID: hosted drift watch + apply ([DEPLOYMENT](DEPLOYMENT.md) §16), provider env / protection, Dashboard profile editor + media publish, `web_order_requests` / real ordering, localization (Q-025), delivery (Q-026). |
| `STOREFRONT-PUBLISH-001` | Dashboard Storefront profile editor + storefront media publication (logo / hero) through one caller-JWT Edge Function running focused, pinned WASM codecs (Option C2, recipe `storefront-media-c4`) | Implemented **locally** on the publication branch `feat/STOREFRONT-PUBLISH-001-v2-focused-codecs` (created from `main` `6cf6c20f`) from the sealed plan (`worktrees/output/storefront-publish-001-plan-20260924T085908Z/`, outside git) as amended by the Option C spike and two owner packets of 2026-09-25 (the Option C full local implementation; the Q031 remediation + correction pass), including the Dashboard Settings → Storefront card. The correction pass replaced the first implementation's ImageMagick engine (recipe c3) with focused jSquash codecs — no copyleft component found by the licence inventory of the vendored files (an engineering inventory, not legal advice), the notices shipped inside the deploy bundle and hash-pinned — and addressed the independent review's findings (`worktrees/output/storefront-publish-001-independent-review-20260925T105015Z/`) in code, tests and docs; the worker-environment check (TNV-9), the strengthened end-to-end assertions (TNV-10) and the SEC-1 refusal of a real GoTrue anonymous session ran in the v2 local end-to-end run ([TESTING_STRATEGY](TESTING_STRATEGY.md) §6). Residuals, each recorded: the gateway half of SEC-2 and a real Dashboard-client publish (hosted canary, [DEPLOYMENT](DEPLOYMENT.md) §17.4 items 4, 5 and 8), the hosted Storage copy / move check (**Q-036**), long-lived worker memory accumulation (not proven locally; a STOP item of the hosted canary, [DEPLOYMENT](DEPLOYMENT.md) §17.4 item 3) and an optional Branding-card warning for receipt logos outside the storefront envelope; the first implementation's branch `feat/STOREFRONT-PUBLISH-001-profile-editor-media` (head `3e0c3ccb`) is evidence only and is **never pushed** (**OPEN QUESTION Q-031**). **Pending independent review of the v2 branch.** **PROPOSED DECISION D-040**; [API_CONTRACT](API_CONTRACT.md) §4.43–§4.44 (with the Dashboard client rules), [STATE_MACHINES](STATE_MACHINES.md) §13, T-018; local evidence in [TESTING_STRATEGY](TESTING_STRATEGY.md) §6 (the `db-tests` job is advisory, so a green run is a pre-merge checklist item). Migration `20260925100000` is **not** applied to hosted and the Edge Function `storefront-media-publish` is **not** deployed; no push, PR, build or provider change. Follow-ups, each separately approved: the owner's publication approval of the v2 branch, the hosted migration PRE / apply / POST, the hosted canary / performance gate and the function deploy ([DEPLOYMENT](DEPLOYMENT.md) §17); **OPEN QUESTION Q-032** (removal of retracted, orphaned and unwanted objects), **Q-033** (a state-aware storage gate), **Q-034** (the READ-001 opening-hours validator defect), **Q-035** (publish blockers for a suspended or soft-deleted tenant and `payload_limit`; the media write path ignores tenant status), **Q-036** (the canonical recipe is enforced by the function, not by the database), **Q-037** (a branch-time-zone pause-end editor; v1 is device-local by owner decision of 2026-09-25, DASH-4 accepted), **Q-038** (server / public-read canonicalization of touching opening-hours windows; the Dashboard's weekly editor refuses them, while touching boundaries through date exceptions and touching periods persisted by Unpublish and save remain accepted residuals); item images as their own Work ID (PUBLISH-002 in the sealed plan). |
| `STOREFRONT-CANARY-GATE-001` | Hosted canary gate of D-040 reconciled with Supabase Edge Runtime semantics (docs, then a test-only follow-up) | The dedicated architecture / contract-change Work ID ([AGENT_WORKFLOW](AGENT_WORKFLOW.md) §9) for [DEPLOYMENT](DEPLOYMENT.md) §17.2–§17.4, **PROPOSED DECISION D-041** (with dated notes in D-040), [API_CONTRACT](API_CONTRACT.md) §4.44, [TESTING_STRATEGY](TESTING_STRATEGY.md) §6, [SECURITY_AND_THREAT_MODEL](SECURITY_AND_THREAT_MODEL.md) §3 Layer 3 and **OPEN QUESTION Q-039**, **Q-040** and **Q-041**; a branch stacked on the publication branch `feat/STOREFRONT-PUBLISH-001-v2-focused-codecs` (PR #288, unmerged), never targeting `main`. Its docs step makes no code, configuration, hosted, data or deploy change; a separately authorized test-only follow-up under this Work ID adds unit test F38 and the Dashboard's platform-failure test (no Dashboard runtime change). The first hosted canary run (2026-09-26) keeps its verdict **NOT PASSED — STOP**; a later canary run is its own owner-named Work ID and runs only on a bundle that carries STOREFRONT-MEDIA-MEMORY-001 and the gate's required test-only guards. |
| `STOREFRONT-MEDIA-MEMORY-001` | Fresh PNG decoder and resize module instances per call in `storefront-media-publish` (**DECISION D-041** point 3) | Planned; its direction was approved by the owner under STOREFRONT-CANARY-GATE-001. A compiled WebAssembly module may stay cached per worker; every call gets fresh instances, so no codec heap or state accumulates across derives. It also adds static guard S8. Vendored bytes, pins and the poisoning / retirement rule are unchanged. Its own tests and independent review — including the local long-lived-worker memory proof of [DEPLOYMENT](DEPLOYMENT.md) §17.4 item 3 — come before any hosted use; it is stacked on the approved STOREFRONT-CANARY-GATE-001 state and fed back into the PR #288 lineage before the next canary. |
