...
- **Key fields:** `id` (UUID PK), `name`, `slug`, `default_currency` (ISO 4217 code — see **OPEN QUESTION Q-007** and [MONEY_AND_TAX_SPEC](MONEY_AND_TAX_SPEC.md)), `country_code` (**OPEN QUESTION Q-001**, drives tax/fiscal config), `status` (`active`/`suspended` — **ASSUMPTION**, org lifecycle is not in the D-018 proposed set), `created_at`, `updated_at`, `deleted_at`. **Self-serve onboarding provenance (RF-090):** nullable `created_by_app_user_id` (the `app_user` who self-served the org) and `creation_request_id` (the client idempotency key), unique per `(created_by_app_user_id, creation_request_id)`; both NULL for orgs not created via self-serve onboarding (see [API_CONTRACT](API_CONTRACT.md) §4.17).
...
> **Pairing states: `paired` vs `active` (DECISION D-034, RF-112).** `paired` = **approved** (a manager approved the enrollment via `approve_device`, `pending → paired`) but **not yet session-eligible**; `active` = **fully provisioned and eligible to open a `device_session`** (§3.5). The `paired → active` edge is the RF-112 **`activate_device`** RPC ([API_CONTRACT](API_CONTRACT.md) §4.28), a **separate** management-authorized step (never inside approval); `start_device_session` (§4.29) requires an **`active`** pairing. `pending → active` is **FORBIDDEN** ([STATE_MACHINES](STATE_MACHINES.md) §9).
...
> **Menu item images (DECISION D-032, RF-110).** Menu item images are **Supabase Storage objects** in the private `menu-images` bucket (object key `{org}/{restaurant}/{branch|global}/menu_item/{menu_item_id}/{image_id}.{ext}`), **not** a `menu_items` column and **not** a `menu_item_images` table. Category/modifier images are **deferred**. RF-111 wires the owner upload UI and the "current image" metadata linkage. Tenant isolation is enforced by **path-derived** `storage.objects` RLS (kitchen excluded; [API_CONTRACT](API_CONTRACT.md) §4.24, [SECURITY_AND_THREAT_MODEL](SECURITY_AND_THREAT_MODEL.md) §14 T-014).
...
- **Access:** RLS enabled + forced with four deny policies and **no role grants**; read only through `public.storefront_menu` (anon, DEFINER, published rows only) and `app.get_restaurant_storefront_profile` (manager+); written only by `app.set_restaurant_storefront_profile` (rank ≥ 2, idempotent, CAS on `version`, audited `settings.storefront.updated` / `settings.storefront.update_denied`). Publish preconditions: slug, branch, resolvable timezone, currency `ILS`, exclusive (or disabled) tax, ≥ 1 live category with a live item, ≥ 1 opening-hours window (**OPEN QUESTION Q-034**: the hours validator accepts entries with missing keys, so a direct caller can satisfy this with `{"weekly":[{}]}`; the STOREFRONT-PUBLISH-001 editor never writes such entries and refuses to publish over saved hours it cannot read).
- **Sync:** not a POS/KDS entity — server-authoritative, never in the outbox; `deleted_at` tombstone kept for the house convention.

### 4.8 `storefront_media` (STOREFRONT-READ-001, PROPOSED DECISION D-039; lifecycle STOREFRONT-PUBLISH-001, PROPOSED DECISION D-040)
- **Purpose:** The map from a **private** original (`menu-images` / `restaurant-logos` object) to its **published public derivative** in the public bucket `storefront-media` (WebP, `w480` / `w960`, ≤ 512 KiB), so that the public read never exposes a private key and a derivative is content-addressed. *(STOREFRONT-PUBLISH-001: the sanctioned path never rewrites a derivative, but the registration-scoped storage policies still let a manager of the restaurant overwrite a registered object through the raw Storage API — **OPEN QUESTION Q-033** — and the database never reads an object's bytes — **OPEN QUESTION Q-036**.)*
- **Key fields:** `id`, `source_bucket ∈ {menu-images, restaurant-logos}`, `source_key` (1–512), `variant ∈ {w480, w960}`, `object_key` (`^[0-9a-f]{32}/[0-9a-f]{64}\.webp$` — the prefix is `app.storefront_media_prefix(restaurant_id)` = the first 32 hex of `sha256('storefront-media:' || restaurant_id)`, the file name is `content_hash`; enforced by CHECK), `content_hash` (the sha-256 the stager declared for the bytes — computed by the Edge Function on the sanctioned path), `width` / `height` (1–4096), `bytes` (1–524288), `published_at`, `unpublished_at` (≥ `published_at` when set), timestamps. Partial-unique `(organization_id, restaurant_id, source_bucket, source_key, variant)` — created by READ-001 `where unpublished_at is null`, narrowed to **LIVE-only** by STOREFRONT-PUBLISH-001 (see *Lifecycle* below); unique `object_key`.
- **Tenant/scoping:** `organization_id` (**D-001**), `restaurant_id`.
...
- **Access:** RLS enabled + forced with four deny policies and **no role grants**. The bucket's SELECT / INSERT / UPDATE / DELETE policies (`to authenticated`) are all gated by `app.can_write_storefront_object(name)` — true only for a **registered** row's `object_key` whose restaurant the caller manages (rank ≥ 2). `public.storefront_menu` emits `image_url` only for rows with `published_at is not null and unpublished_at is null`. The generation / upload / publish action is a later Dashboard ticket; no upload, delete, signed URL or export exists in this slice. *(STOREFRONT-PUBLISH-001 adds that generation / upload / publish path — see the Lifecycle bullet below — and still no delete, signed URL or export. The policies are registration-scoped, not state-scoped: **OPEN QUESTION Q-033**.)*
- **Lifecycle (STOREFRONT-PUBLISH-001, PROPOSED DECISION D-040; transitions [STATE_MACHINES](STATE_MACHINES.md) §13; write path [API_CONTRACT](API_CONTRACT.md) §4.43–§4.44).** The state is **derived** from the two timestamps; there is no status column:
  - **STAGED** — `published_at is null`: registered at its content address but **not referenced by the Storefront public data contract** (the profile writer refuses to point at it and `public.storefront_menu` never serves it). The bucket is public, so STAGED means *unreferenced*, not *private*: an uploaded object is fetchable by anyone holding its URL from the moment of upload.
  - **LIVE** — `published_at` set, `unpublished_at` null: the only state a profile pointer (`logo_media_id` / `hero_media_id`) may reference — the profile writer accepts only LIVE rows of the same restaurant — and the only state the public read serves.
  - **RETRACTED** — `unpublished_at` set: no longer referenceable or served; its object stays fetchable by URL.
  - **Transitions** (who drives each, conditions and the forbidden edges: [STATE_MACHINES](STATE_MACHINES.md) §13): STAGED → LIVE (`finalize`), STAGED → row deleted (`cancel`), LIVE → RETRACTED (`retract`, or a same-source replacement inside `finalize`), and **RETRACTED → LIVE** (`finalize` again, reported as `republished`) — required because re-deriving the same bytes lands on the same `object_key`, which `unique (object_key)` forbids registering twice. `finalize` checks that an object **row** exists under the key with declared type `image/webp` and exactly the staged size, on every call (not the bytes: **OPEN QUESTION Q-036**).
  - **LIVE-only live-source index:** `(organization_id, restaurant_id, source_bucket, source_key, variant) where published_at is not null and unpublished_at is null` — at most one LIVE derivative per (restaurant, source, variant), while a STAGED replacement (and several STAGED attempts of one source) can coexist with the LIVE row it will replace.
  - **One derivative per bytes per restaurant:** the object key is the content address `<prefix>/<sha256>.webp`, so staging identical bytes again — even from another source — returns the existing row, which reports its own source (`existing`; `source_bucket` / `source_key` / `variant` are the row's, with `source_mismatch` when they differ from the request). Finalizing such a row publishes it as a derivative of **its own** source ([API_CONTRACT](API_CONTRACT.md) §4.43.1).
  - **Replacement re-point:** finalizing a new derivative of a source whose LIVE row a profile slot uses retracts the old row and moves the slot to the new row **in the same transaction** (profile `version + 1`, audit reason `media_replaced`) — a slot never points at nothing.
  - **No object deletion:** no path deletes a `storefront-media` object. `cancel` removes the STAGED **row** only (an uploaded object stays at its content address, unregistered — the tenant can then no longer write or delete it — and is reused by a later stage of the same bytes); `retract` keeps the object, which stays reachable at its public URL. Removal is **OPEN QUESTION Q-032**.
  - **Scope:** logo = `w480` from `restaurant-logos`; hero = `w960` from `restaurant-logos` or `menu-images`; `(menu-images, w480)` — the pair the public read joins for item `image_url` — is refused (`variant_not_allowed`), so item images stay a later Work ID.
