...
**Takeaway rule (RESTAURANT-OPERATIONS-V1-001, review B3):** takeaway/pickup orders follow the **same** chain — `ready → served → completed`. `served` for a takeaway order **is the customer pickup**; the POS/KDS/Dashboard *display* it as **"Picked up"**, but the persisted state is `served` and there is **no** `picked_up` state. The pre-implementation draft of this rule ("takeaway skips `served`, `ready → completed`") is **superseded**: `ready → completed` is not a legal direct transition for any order type (the server's auto-completion of a served+settled order is a settlement **side effect** — ORDER-AUTO-COMPLETION-001 below — never a manual transition). Authoritative RPC behaviour: [API_CONTRACT.md](API_CONTRACT.md) §4.32.
...
| ready → served | cashier / manager / server | **any order type** (review B3): dine-in = served at the table; takeaway = customer pickup (displayed "Picked up") | No | Always | Yes-provisional | No |
| served → completed | cashier / manager / server | **any order type**; payment settled per money spec (see [MONEY_AND_TAX_SPEC.md](MONEY_AND_TAX_SPEC.md)) | No | Always | Yes-provisional | No |
...
> 1. **The payment gate was never enforced.** D-025 above requires that an order reach `completed` only when payment is `completed`, but `app.update_order_status` contained **no payment check** — an UNPAID order could be completed. The state machine now enforces it: completing an order with no `completed` payment is refused with the stable domain error **`order_not_paid`**, and nothing is written. This affects **only** the `→ completed` transition, which no client previously performed, so no shipped behaviour regressed. **Caveat, since RESOLVED:** D-025 says *"for a **chargeable** order"*, and when this note was written **no chargeable flag existed in the schema** (the word appeared only in prose), so the gate applied to **every** order. **ORDER-AUTO-COMPLETION-001 gave it a definition — `grand_total_minor > 0` IS "chargeable"** — and a **zero-total** order is therefore NON-CHARGEABLE: it owes nothing, settles with no payment row, and `app.record_payment` now refuses to charge it at all (`order_not_chargeable`). See the note below and [API_CONTRACT §4.32d/§4.34](API_CONTRACT.md). Completion still creates **no** payment and changes **no** payments row — payment and fulfillment remain independent axes; the gate only makes fulfillment *wait*.
...
>
> **RESOLVED (RESTAURANT-OPERATIONS-V1-001, review B3):** the **takeaway** fork this note used to flag is gone. The shipped RPC behaviour — one shared chain, `ready → served` permitted for every order type, and — for every order that travels the production/served chain — `served → completed` being the route to `completed` (the zero-total `printer_only` submit exception in §1.2 is the one authorized path that never enters that chain) — **is** the canonical model, and this document, the shared Dart domain (`packages/domain` `OrderStateMachine`), and [API_CONTRACT.md](API_CONTRACT.md) §4.32 now all state it identically. A takeaway `served` is the customer **pickup** (display wording "Picked up"; no `picked_up` state).
...
>   - **Consequence — DECIDED, not left open (MONEY-SETTLEMENT-CONSISTENCY-001):** a comped order that auto-completes becomes **terminal and therefore NON-VOIDABLE**, where before it was voidable (it carries no completed payment, so the RF-062 void guard never fired). The human decision is that **`completed` stays TERMINAL, exactly like any other completed order — there is NO special `completed → void` path for a zero-total order** (that would be the first crack in D-024, and post-completion correction is a policy question of its own). What changed instead is the **surface**: the POS no longer offers a Cancel button on an order it knows is terminal, and no longer gates that button on the payment marker. See [API_CONTRACT §4.34](API_CONTRACT.md).
...
> **IMPLEMENTATION NOTE (MONEY-SETTLEMENT-CONSISTENCY-001) — the DISCOUNT axis is now frozen at payment.** **DECISION D-025** keeps payment and fulfillment independent, which means a fully-PAID order is still `submitted`/`served` — **not terminal** — and `app.apply_discount` was guarded **only** by terminal status. It could therefore re-base `grand_total_minor` **underneath a settled payment**. Now: an order carrying a **live completed payment** refuses **any** discount mutation, in **either** direction (raising *or* lowering the total — after the guest is charged, a price change is a **refund**, and **D-023** makes a completed payment TERMINAL). The order row is locked `FOR UPDATE` first, so a payment cannot race the guard. A **non-chargeable** zero-total order carries no payment and stays discountable. No lifecycle state or transition changed. See [API_CONTRACT §4.5 and §4.34](API_CONTRACT.md).
...
- `submitted → completed` — **FORBIDDEN as a general edge**, with ONE narrow exception documented below: a **zero-total order on a `printer_only` branch**, which `app.submit_order` settles inside the submit transaction itself (a zero balance is already settled, **D-025**). No new state is introduced.
- `submitted → served` — **FORBIDDEN as a general edge**, with ONE narrow, server-only exception documented below (**KITCHEN-PRINT-DUAL-001C** / **KITCHEN-DISPATCH-ENFORCE-001**). No state is added or renamed by that exception (**D-018** is intact); it is a *dispatch* route, not a new lifecycle stage.

> **IMPLEMENTATION NOTE (KITCHEN-PRINT-DUAL-001C + KITCHEN-DISPATCH-ENFORCE-001) — the `printer_only` dispatch route, and the ONE legal `submitted → served` edge.** This doc predated the dual kitchen-workflow model and forbade `submitted → served` outright while the shipped code already performed it; that contradiction is resolved here in favour of the implemented, now guard-enforced behaviour.
> - **Two orthogonal axes.** `branches.kitchen_workflow_mode ∈ {kds, printer_only}` is how a BRANCH consumes orders; `orders.dispatch_mode ∈ {kds, direct_print}` is how THAT order was dispatched. They are separate columns with separate CHECKs.
> - **The edge.** On a `printer_only` branch the kitchen ticket **is** the printed POS ticket — there is no KDS device to acknowledge/prepare/ready it. `app.apply_direct_print_dispatch` therefore promotes the freshly `submitted` order straight to `served` in the SAME `app.sync_push` transaction, stamps `dispatch_mode='direct_print'` + a write-once `ready_at`, serves its items and writes ONE money-free `order.status_updated` audit. A concurrent `sync_pull` can never observe an intermediate active state.
> - **This is the ONLY legal route to that edge.** It is server-internal (`app.apply_direct_print_dispatch` is granted to no client role) and it never fires on a `kds` branch.
> - **It is ENFORCED at ingest, not merely requested (KITCHEN-DISPATCH-ENFORCE-001).** `app.sync_push` validates the requested `dispatch_mode` against the AUTHORITATIVE `branches.kitchen_workflow_mode` — read from the SESSION-derived scope, never the payload — BEFORE `app.submit_order` runs, so a refusal creates no order at all. `direct_print` requires an affirmative `printer_only` read (taken `FOR SHARE`, which serializes acceptance against concurrent mode changes); anything else — including a missing or tombstoned branch row — fails CLOSED with the terminal typed error `dispatch_mode_not_allowed`. An unknown dispatch value is `invalid_payload`. A defensive belt inside `app.apply_direct_print_dispatch` re-proves the branch mode and raises a dedicated internal SQLSTATE that `sync_push` normalizes to the same typed error, so no raw SQLSTATE or branch-mode detail ever reaches a client. Before this, enforcement was CLIENT-ONLY and a forged `direct_print` on a KDS branch could strand an order invisible to both the board and the printer.
> - **Compatibility.** A client that OMITS `dispatch_mode` (every pre-001C build, and the modern POS on a kds branch) is defaulted to `kds` and accepted on EVERY branch mode; on a `printer_only` branch such a **chargeable** order stays `submitted` and still gets its kitchen dispatch row from `app.submit_order`'s branch-mode gate — but a **zero-total** one completes immediately (next bullet).
> - **Accepting `direct_print` does NOT mean the order ends `served`.** On a `printer_only` branch `app.submit_order` itself creates the kitchen dispatch row AND auto-completes a **zero-total** order, because a zero balance is already settled (**D-025**). By the time `app.apply_direct_print_dispatch` runs, that order is terminal, so it correctly declines with `dispatched=false` / `not_eligible`. Final states on a `printer_only` branch:
>   - `direct_print` + **chargeable** → `served`, `dispatch_mode='direct_print'`, revision 2, `dispatched=true`; completes later on settlement.
>   - `direct_print` + **zero-total** → **`completed`, `dispatch_mode='kds'`, revision 2, `dispatched=false`** — the `submitted → served` promotion never runs; this is the `submitted → completed` exception above.
>   - absent/`'kds'` + chargeable → stays `submitted`; completes on settlement.
>   - absent/`'kds'` + zero-total → **`completed`** inside the submit transaction.
> - **Completion is always settlement-driven, never print-driven.** A served order completes ONLY through the normal settlement rule (**D-025**, `app.order_is_fully_settled`) — never as a side effect of a successful print; the zero-total case is the same rule applied to a zero balance, not a second exception. The kitchen dispatch LEDGER row (written by `app.submit_order`), `apply_direct_print_dispatch`'s `dispatched` outcome, and the POS's own local physical print are three distinct things. This lifecycle predates KITCHEN-DISPATCH-ENFORCE-001 and is unchanged by it. See [API_CONTRACT §4.41](API_CONTRACT.md).
...
- `ready → completed` as a **direct** transition — **FORBIDDEN for every order type** (review B3). For any order that entered a production/served path — including a **CHARGEABLE** `direct_print` order — the legal completion transition is `served → completed`, applied **after settlement**; the system's auto-completion of a served + fully-settled order (ORDER-AUTO-COMPLETION-001 above) chains that already-legal single step as a settlement side effect, is not a `ready → completed` edge and never skips `served`. **The one authorized exception to "completion is reached via `served`" is the ZERO-TOTAL `printer_only` submit** documented above: `app.submit_order` completes such an order directly `submitted → completed`, because its balance is already settled (D-025) — it never reaches `served` and never reaches the direct-print promotion. **Physical print success is not the completion trigger on either path, and no other `submitted → completed` transition is authorized by that exception.**
