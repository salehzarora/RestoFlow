...
- `submitted → completed` — **FORBIDDEN** (must pass production states).
...
> - **Compatibility.** A client that OMITS `dispatch_mode` (every pre-001C build, and the modern POS on a kds branch) is defaulted to `kds` and accepted on EVERY branch mode; on a `printer_only` branch such an order stays `submitted` and still gets its kitchen dispatch row from `app.submit_order`'s branch-mode gate.
> - **Completion is unchanged.** The served order completes ONLY through the normal settlement rule (**D-025**, `app.order_is_fully_settled`) — never as a side effect of a successful print. A zero-total order on a `printer_only` branch is settled-with-nothing-to-pay and auto-completes inside the submit transaction itself, before any dispatch runs. See [API_CONTRACT §4.40](API_CONTRACT.md).
