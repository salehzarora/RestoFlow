...
>
> **RESOLVED (RESTAURANT-OPERATIONS-V1-001, review B3):** the **takeaway** fork this note used to flag is gone. The shipped RPC behaviour — one shared chain, `ready → served` permitted for every order type, the only route to `completed` being `served → completed` — **is** the canonical model, and this document, the shared Dart domain (`packages/domain` `OrderStateMachine`), and [API_CONTRACT.md](API_CONTRACT.md) §4.32 now all state it identically. A takeaway `served` is the customer **pickup** (display wording "Picked up"; no `picked_up` state).
...
- `ready → completed` as a **direct** transition — **FORBIDDEN for every order type** (review B3): the only route to `completed` is `served → completed`. The system's auto-completion of a served + fully-settled order (ORDER-AUTO-COMPLETION-001 above) chains that already-legal single step as a settlement side effect; it is not a `ready → completed` edge and never skips `served`.
