...
2. **CPU headroom** — the worst call, measured on the platform under overlapping load (the measurement method is recorded with the evidence), is **≤ 75 %** of the 2 s per-request CPU limit, i.e. ≤ 1,500 ms. Local reference (edge runtime `v1.74.3`, wall time per derive): the slowest vector of the final parity run 283 ms; the accepted worst layouts above, worst derive 373 ms in a reused 256 MB worker (120 derives, 0 kills) and 341 ms in a fresh one (worst round trip 956 ms).
3. **Gateway and authentication (review SEC-2)** — a genuine signed-in manager's session passes the gateway and publishes; the gateway `verify_jwt` layer refuses a request with no token and one with a foreign token (another project's, or forged) before the function runs; the legacy anon key used as the bearer, the token of a signed-out session and an anonymous device session's JWT reach the function and are refused `401 unauthenticated` by it (SEC-1); and a browser `OPTIONS` preflight from both production Dashboard origins (`https://app.bizbot.systems` and `https://resto-flow-phi.vercel.app`, §5.1) succeeds under `verify_jwt = true`. The gateway layer was not exercised locally (the evidence-branch end-to-end harness substituted the gateway).
4. **A real client publish (review CRIT-3)** — one publish through the canary from the Dashboard's own Dart client (`SupabaseStorefrontFunctionInvoker`), preferably from a Dashboard build in a browser at a production origin, covering the request shape, the CORS headers, the ladder and the slot assignment end to end (the Dashboard-to-function wire path was otherwise proven only against test doubles).
5. **Kill convergence** — a request killed mid-flight and retried with the same `request_id` converges to exactly one LIVE row and one object. Local reference: on the evidence branch (recipe c3, the same workflow), a CPU-limit kill before `stage` and three edge-runtime container kills with the request already inside `stage` or `finalize` (parked in `stage` and then let through; parked in `finalize` and then either cancelled or committed) each converged; the runtime's wall-clock limit did not end the short local requests, so those kills were forced by parking the request on a database row lock. The v2 local end-to-end result is recorded in [TESTING_STRATEGY](TESTING_STRATEGY.md) §6.
6. **Egress per invocation (review CRIT-4)** — the Storage egress of one full ladder (five rungs) and of one retry is measured and recorded against the quota recorded in the §17.2 PRE.
7. **Removal** — the canary deployment is deleted afterwards. Its synthetic tenant, originals and derivatives are removed under the same approval, which names the removal method, because no application path deletes a `storefront-media` object (**OPEN QUESTION Q-032**); its append-only `audit_events` rows remain (**DECISION D-013**).

Any miss is **STOP**: no deploy and no Dashboard publication. Widening the c4 envelope or changing the recipe to fit is a new owner decision (D-040 points 1 and 3).
