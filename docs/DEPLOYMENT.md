...
7. **Secrets and configuration (names only)** — list, by **name only**, the Edge Function secrets and the project configuration relevant to this function; prove that no `service_role` key and no S3 access key is configured for, or read by, this function (static guard S2 already refuses any read), and that `SUPABASE_URL` and `SUPABASE_ANON_KEY` are the only settings it reads. **Never record a secret value.**
