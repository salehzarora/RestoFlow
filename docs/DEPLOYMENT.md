...
2. **Hosted canary / performance gate** (§17.4) — a time-boxed deployment of the function; REQUIRED before any publication or deploy (owner packet, 2026-09-25).
...
- **Gate:** nothing here runs before §17.1 step 0 (the owner's publication approval, with the notices and pins checked). The canary of §17.4 uses this recipe for its time-boxed deployment; the function deploy of §17.1 step 3 follows only once the canary evidence is sealed.
...
- **Environment:** the function reads only `SUPABASE_URL` and `SUPABASE_ANON_KEY`; set **no** secret for it. Before deploying, confirm **by name only** (never print or record a value) that the hosted function environment still provides `SUPABASE_ANON_KEY`, the legacy anon key, and not only the newer publishable key; if it does not, **STOP** — changing the key source is a code change under its own review. The platform's privileged defaults (`SUPABASE_SECRET_KEYS` and the legacy `SUPABASE_SERVICE_ROLE_KEY`, present in every hosted function environment) are never read or used by this function (static guards S1 / S2), and no change may add a read of them.

### 17.4 Hosted canary / performance gate (REQUIRED before any publication or deploy)

A **time-boxed** deployment of the function on the hosted project (recipe §17.3; it is itself a deployment of the v2 bundle, so step 0 of §17.1 comes first), exercised with a synthetic tenant, must show every item below; its evidence is sealed before step 3 of §17.1:
...
2. **CPU headroom** — the worst call, measured on the platform under overlapping load (the measurement method is recorded with the evidence), is **≤ 75 %** of the 2 s per-request CPU limit, i.e. ≤ 1,500 ms. Local reference (edge runtime `v1.74.3`, wall time per derive): the slowest vector of the final parity run 283 ms; the accepted worst layouts above, worst derive 373 ms (120 derives spread over at least 54 short-lived worker instances, not one reused worker — see item 3) and 341 ms in a fresh worker per source (worst round trip 956 ms).
3. **Long-lived worker memory (STOP; review V-2 of the correction delta review)** — locally the runtime never kept one worker for more than 6 derives (at least 54 instances, 64 early terminations), so memory accumulation across one long-lived worker is **not proven**. On the canary: run a bounded repeated-derive sequence (the accepted worst layouts of item 1, in the order 8-bit RGB PNG at 8 MiP, 8-bit RGBA Adam7-interlaced PNG at 8 MiP (3456 × 2427), 16-bit RGBA PNG at about 4 MiP, progressive 4:4:4 JPEG at about 5.3 MiP, progressive 4:2:0 JPEG at 8 MiP, repeated at least five times, each for both variants `w480` and `w960`); inspect the function logs and the shutdown metadata the platform exposes (shutdown reason, `WorkerMemoryUsed` or its equivalent, CPU time); record the external / total memory trend per worker. Pass only with **zero** Memory or CPUTime shutdowns for the sequence and no rising trend. If the platform does not reuse a worker long enough to assess accumulation, record long-lived-worker accumulation as **UNPROVEN** and **STOP** before any production publication — never record it as passed.
4. **Gateway, JWT signing and CORS (review SEC-2)** — record the hosted JWT signing mode (legacy shared secret or asymmetric signing keys) and prove that a genuine signed-in session passes the gateway `verify_jwt` check under it; the gateway refuses a request with no token and one with a foreign token (another project's, or forged) before the function runs; a browser `OPTIONS` preflight from both production Dashboard origins (`https://app.bizbot.systems` and `https://resto-flow-phi.vercel.app`, §5.1) succeeds under `verify_jwt = true` with the contracted CORS headers ([API_CONTRACT](API_CONTRACT.md) §4.44): `Access-Control-Allow-Origin: *`, `Access-Control-Allow-Methods: POST, OPTIONS`, `Access-Control-Allow-Headers: authorization, x-client-info, apikey, content-type`, and **no** `Access-Control-Allow-Credentials` — the calls are bearer-token only and use no cookies, so the wildcard origin is expected, not a failure (an origin allowlist would be its own Work ID with a contract change); an unauthorized or signed-out request (no token, or a refused token) still gets its authentication / authorization refusal, with the same CORS headers so the Dashboard can read it. The gateway layer was not exercised locally (the evidence-branch end-to-end harness substituted the gateway).
5. **Authentication and authority (reviews SEC-1 / SEC-2 / R-003)** — against the hosted function and database, with synthetic tenants A and B:
   ...
   - the legacy anon key as the bearer, the token of a signed-out session and an anonymous device / kiosk session's JWT reach the function and are refused `401 unauthenticated` (SEC-1);
   - every Storage object written is owned by the **caller's** auth user and every RPC runs as the caller (caller-context Storage / RPC), and no request uses a `service_role` key.
6. **Function logs and shutdowns** — the hosted function logs for the whole canary are inspected: they carry error names only (never a key, token, e-mail address or request data); every shutdown is listed with its reason, and any Memory or CPUTime shutdown outside an intentional kill is a STOP; worker retirement appears only after an induced engine failure; the CPU time per request of item 2 is read from the platform's metrics or logs, not estimated.
...
8. **A real client publish (review CRIT-3)** — one publish through the canary from the Dashboard's own Dart client (`SupabaseStorefrontFunctionInvoker`), preferably from a Dashboard build in a browser at a production origin, covering the request shape, the CORS headers, the ladder and the slot assignment end to end (the Dashboard-to-function wire path was otherwise proven only against test doubles).
9. **Kill convergence and reconciliation** — a request killed mid-flight (where it can be done safely on the canary, for example by an induced CPU-limit kill before `stage`) and retried with the same `request_id` converges to exactly one LIVE row and one object; after an unknown outcome the Dashboard's reconciliation (`list_storefront_media` and a profile re-read) shows the authoritative state. Local reference: on the evidence branch (recipe c3, the same workflow), a CPU-limit kill before `stage` and three edge-runtime container kills with the request already inside `stage` or `finalize` (parked in `stage` and then let through; parked in `finalize` and then either cancelled or committed) each converged; the runtime's wall-clock limit did not end the short local requests, so those kills were forced by parking the request on a database row lock. The v2 local end-to-end result is recorded in [TESTING_STRATEGY](TESTING_STRATEGY.md) §6.
...
Items 1–11 are all required before any production release or publication. Any miss is **STOP**: no deploy and no Dashboard publication. Widening the c4 envelope or changing the recipe to fit is a new owner decision (D-040 points 1 and 3).
