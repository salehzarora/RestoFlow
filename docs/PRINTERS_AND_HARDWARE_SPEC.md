...
- **Duplicate-kick prevention:** a `drawer_kick` is NOT auto-retried on uncertain outcome (a second pulse would re-open an already-open drawer pointlessly and confuse cash handling). It carries an idempotency key (**D-022**) and is single-flight (Section 8.3). Repeated user requests within a short window are de-bounced.
