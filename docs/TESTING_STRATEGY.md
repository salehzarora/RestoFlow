...
- The Postgres role `anon` (publishable key, no session) can execute **no** `public` function outside the explicit allowlist (**empty** at Phase 1A; **exactly `storefront_menu(text)`** since STOREFRONT-READ-001 — **DECISION D-037** as amended, **DECISION D-039**; that function's own contract is T-017), reads **no** row from any `public` table or view (refused at the grant layer, before RLS), and cannot use schema `app`; asserted as the **real** role (`set local role anon`), never via faked JWT claims (T-016).
...
- **Storefront public read** (T-017): run `public.storefront_menu(text)` as the real `anon` role **and** as a real `authenticated` principal (`42501`): exact served-key sets per entity, cross-tenant / sibling-branch isolation, the byte-identical `not_found` for every unpublished / unknown / invalid case, the typed `payload_limit`, `prosrc` free of membership helpers, and the profile / media tables RLS-forced with no grants (`supabase/tests/storefront_read_001_test.sql`); the browser half runs against a real `next start` of a live-mode build (`storefront/tests/browser/storefront-read-001.spec.ts`).
- **Storefront media publication** (T-018, **DECISION D-040**): run the five media RPCs as the real `authenticated` role with the identity GUC, and as the real `anon` role for the grant-layer refusal (its message pinned, so a body refusal cannot pass for it): wrapper / body / grant introspection on both layers, the anon allowlist and `public` `SECURITY DEFINER` set still exactly `storefront_menu(text)`, the LIVE-only index, exact enumerations (prototype names included), the private-original source proof, rank 0 / 1 / cross-tenant authority (a foreign or unknown restaurant refused with `42501` before any input validation — for an org owner and an org-wide cashier too — and never named by an audit row), idempotency (a `stage` replay reporting the row's current state, `stale_request` after a cancel, a `finalize` replay after a retract answering `stale_request` and changing nothing, typed refusals never claimed), STAGED absent from the public read (also when a profile pointer or an item-slot row is forced onto it, with a discriminating LIVE control), the finalize object check, the atomic same-source re-point (logo, hero, or both slots in one version bump; none when no slot uses the replaced row; a replaced row whose `published_at` lies ahead of `now()`), RETRACTED → LIVE, retract / cancel rules with no object ever deleted, the uniform `not_found` of `list`, the audit trail, and — added by the correction pass — section J: the tenant binding and authority of `cancel`, `retract` and `finalize` (foreign media ids → `not_found` with the foreign rows untouched and no audit row carrying them; covering cashier → audited typed denial; sibling manager / foreign owner → `42501`; a foreign restaurant named by an org owner or an org-wide cashier → `42501` with no audit row), `cancel` of a LIVE row refused, the finalize object check on a replay and on an already-LIVE row against the real SQL (deleted, wrong size, restored), and the `retract` replay re-read (`stale_request` once the row is LIVE again or gone) (`supabase/tests/storefront_publish_001_test.sql`, `plan(163)`: sections A 16, B 30, C 7, D 15, E 13, F 16, G 8, H 5, I 18, J 35). Real two-session concurrency — double finalize, re-point versus a stale CAS save, cancel versus finalize, double stage, and two different STAGED rows of one source finalized concurrently (the loser blocks, then replaces the winner; one LIVE row remains) — runs on separate dblink sessions in the accepted `zz_` harness (`supabase/tests/zz_storefront_publish_001_concurrency_test.sql`, `plan(23)`, scenarios H1–H5; committed, self-cleaning fixtures). Both files run in the existing `db-tests` job (`supabase test db`). **That job is advisory, not a required check** (review CRIT-1): per the independent review's read of branch protection, the only required status check is the `validate` job, so the §8 hard gate for these suites is enforced by process on this change, not by branch protection — **pre-merge checklist item for this PR: a green `db-tests` run on the PR head (or, failing that, the local CI replica below) covering T-016, T-017, T-018, the `zz_` file and RF-019.** Changing branch protection is an owner / provider setting and is not part of this change. Results on the v2 branch (Phase E): T-018 163/163; `zz_` 23/23; T-016 62/62; T-017 121/121; RF-019 15/15; CI replica (a throwaway stack: a fresh `supabase start`, `supabase db reset` of all 142 migrations, `supabase test db`) 276 files, 7,170 tests, PASS, at `c7603ac8` (no container or volume left).
- **Storefront media Edge Function** (Node, no Docker, no network, no secret): `node --test` of the six files `supabase/functions/storefront-media-publish/test/{validation,recipe,handler,engine_poison,license,static_guard}.test.mjs` against the same vendored, sha-pinned codecs the function loads — validation (exact allowlists incl. every prototype-chain name, the exact request key set, a top-level key named twice, ids, key grammars, the storage-key encoder's dot segments, the publisher-principal check, rung); recipe (the codec pins checked before anything is compiled, the EDGE-4 init self-test and its fail-closed mismatch, the c4 constants, exact integer geometry and box pre-reduction, the goldens of `test/goldens.mjs` byte for byte, determinism across fresh codec instances, every hostile vector's typed refusal, a 4-component JPEG refused before decode, the alpha floor of the ladder, the EDGE-1 PNG image-data bound — a 256 MiB compressible tail refused within one chunk of the raw size, the zlib wrapper checks per runtime (the Node 22 residual pinned), exact raw sizes incl. Adam7, ancillary chunks inflating to 1 GiB never reaching the decoder — the 8 MiP edge (4096 × 2048 derives, one row more is `too_many_pixels`), the two decoder memory budgets checked from the headers before any inflate or decode (R14b: PNG image data ≤ 33,570,816 bytes — a 16-bit RGBA 2049 × 2048 PNG refused, plain and Adam7, 2048 × 2048 admitted, every 8-bit RGBA shape at the pixel cap admitted, plain and Adam7 — and progressive-JPEG coefficients ≤ 32 MiB — 4:4:4 at 8 MiP refused, 4:2:0 at 8 MiP admitted), alpha carried exactly, the eight EXIF orientations, metadata stripped, codec failures mapped to the public codes, the output self-check); handler (transport guards, authentication first and the SEC-1 refusal of anonymous / non-`authenticated` principals, validation before any lookup, the preflight before any download, the caller-only happy path, the ladder, every typed status mapping, the streaming byte cap measured in bytes read, recovery after a kill before or during stage, upload and finalize, an abandoned attempt, object conflict — also a same-length foreign object — key reuse, the defence-in-depth self-checks, engine retirement, no request data in logs, `409 restart_required` paths, Storage errors classified by their JSON body, a LIVE row whose object vanished healed by the next publish, a concurrent upload race resolved by read-back and comparison — against a test double of Auth / PostgREST / Storage that refuses any method but `GET` / `POST`); engine_poison (a PNG decoder exception: `422 decode_failed`, the engine poisoned, the worker retired, later requests never reaching the engine — in its own process); license (every vendored file and the notices byte-exact to their pinned sha-256, the exact vendor file set, the notices and README contents, the `static_files` list, the `.gitattributes` rules, and no path or file naming the previous engine); static guard (S1–S7: no privileged credential, every `Deno.env` token counted and exactly two environment reads, relative imports only, S4 an allowlist of `GET` / `POST` and exactly the two Storage routes with `x-upsert` only `'false'`, `verify_jwt = true` and the exact static files, no raw bytes or isolate state in responses). CI runs it as the step "Tests — storefront media Edge Function (Node, pinned wasm)" in the existing `validate` job (Node from `storefront/.nvmrc`, 24; the `on:` block is unchanged). Results on the v2 branch (Phase E): Node 24.21.0 (Linux container) 165/165 — validation 17, recipe 94, handler 39, engine_poison 1, license 6, static_guard 8; Node 22.14.0 (Windows) 165/165 with the same per-file counts, both at `c7603ac8`. **Edge-runtime parity (a local gate, not CI):** every golden and refusal of `test/goldens.mjs` and the EDGE-4 self-test derived through the function's own `createDeriver` on Node 22, Node 24 and the Supabase edge runtime `v1.74.3` (a fresh 256 MB user worker per vector, reading only the configured static files) must be identical: 74/74 vectors identical (50 derived goldens and 24 typed refusals; the slowest edge call 283 ms), run at `4a37e612` — the recipe and the codec files are unchanged since.
- **Storefront media end-to-end — a publication gate, not a CI lane.** The `validate` job is Docker-free, so the function never runs on the edge runtime in CI. A local end-to-end run of the real function on the Supabase edge runtime (`v1.74.3`) against a throwaway local stack's Auth / PostgREST / Storage, with a synthetic tenant that is removed afterwards, is required before publication, and the hosted canary of [DEPLOYMENT](DEPLOYMENT.md) §17.4 before any deploy. Result on the v2 branch (Phase E): 54/54 at `c7603ac8` — goldens through the real function equal to the Node deriver and the committed c4 goldens, objects owned by the caller's auth user, a real GoTrue anonymous session refused `401` (SEC-1), a duplicate top-level key refused (SEC-4), refusals leaving no row, the 8 MiP cap (one row over refused, exactly at the cap derived), the EDGE-1 bomb refused `corrupt`, abandoned-attempt recovery, retract / replay / republish, the live storefront render and its `404` after unpublish, three kill-convergence runs, the worker-visible environment holding no privileged value (TNV-9), and the synthetic tenant removed (the stack's schema came from the same migration, whose only later change is a two-line comment). History: the evidence branch's run (recipe c3, the same workflow, 2026-09-25) passed 49/49, including a CPU-limit kill before `stage` and three edge-runtime container kills with the request parked inside `stage` or `finalize` on a database row lock, each converging to one LIVE row and one object on a retry with the same `request_id`. **Harness notes (the evidence-branch harness, not product code):** it substituted the gateway, so the gateway `verify_jwt` layer was not exercised locally (a hosted canary item, [DEPLOYMENT](DEPLOYMENT.md) §17.4), and the runtime's wall-clock worker limit did not terminate short local requests, hence the lock-parking method for kills. End-to-end harnesses are scratch-only; their results belong in the ticket's sealed evidence pack.
- **Storefront editor** (Dashboard widget and unit tests, **DECISION D-040** point 10): 12 files `apps/dashboard/test/storefront_*_test.dart` — the profile and media repositories (read before edit, CAS save, one request id per logical save with one same-id retry and readback reconciliation, list / retract / cancel, a retract replay answered `stale_request`), the publisher (the eight-key request, the rung loop, typed outcomes only for contract bodies, one same-id retry then an unknown outcome, the hard cap, `401` → sign in again for an anonymous session, `content_mismatch` answered as `409 object_conflict`), the opening-hours grammar and editor (overlap warnings, touching windows refused — same day, across midnight, Saturday into Sunday, a close of 00:00, a minute apart allowed — with Save and Publish disabled and the ar / he / en copy, the **Add hours** seed, unreadable saved hours kept verbatim), contrast, sources (branch status), the section and the media slot (including the card kept alive when it scrolls out of the Settings list and a fresh editor on a scope change, the same-request retry of an unknown slot assignment, a deterministic slot refusal with its reason and blockers, the receipt-logo re-check at Publish, a refused receipt logo keeping the logo slot's Publish disabled with its remedy, a failed source read, the published status against the public read's `not_found` blockers — the not-found conditions are read from the READ-001 SQL source —, the availability note, a suspended branch marked, a past pause end, a name-only save sending exactly that key, and Discard after an unknown save), the real Dashboard shell handing the editor its own membership's scope identity for three different memberships (DASH-V-1), the availability note on a published storefront that is also offline or incomplete (DASH-V-2), a 360 px layout, and ar / he / en. Each of `app_en.arb`, `app_ar.arb` and `app_he.arb` carries the same 263 `storefront*` keys. Results on the v2 branch (Phase E): the 12 files 272/272 at `c7603ac8`; the full `flutter test apps/dashboard` 2,073 passed / 16 skipped / 0 failed at `c7603ac8`; `flutter test packages/l10n` 80/80 at `c7603ac8`. After the final four-finding correction pass (touching opening-hours windows, Q-038): the 12 files 283/283, the full Dashboard 2,084 passed / 16 skipped / 0 failed, `packages/l10n` 80/80.
...
1. **Authorization**: callers without the required membership/role/scope are denied; callers with it succeed (deny-by-default); the Postgres role `anon` is denied at the grant layer unless the RPC is on the D-037 allowlist (T-016); an allowlisted anon-only RPC is in turn denied to `authenticated` and is proven as both real roles (T-017).
...
2. **RLS harness (Section 6)** including the default-deny presence check for new tables. In CI these suites run in the `db-tests` job, which is advisory — only `validate` is a required status check (review CRIT-1) — so for STOREFRONT-PUBLISH-001 a green `db-tests` run on the PR head, or the local CI replica, is a written pre-merge checklist item (§6; **DECISION D-040** point 12).
