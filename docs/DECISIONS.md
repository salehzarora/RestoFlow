...
- **Decision:** **DECISION D-035** —
  ...
  - **Grant posture mirrors the app RPC exactly:** `revoke all ... from public` + `grant execute ... to authenticated` — never `anon`/`service_role` (**D-011**); the `anon` revoke must be **explicit** — hosted Supabase's `ALTER DEFAULT PRIVILEGES` stamps every new `public` function with a separate `anon` grant that `REVOKE … FROM PUBLIC` does not remove (corrected by **D-037** / STOREFRONT-SEC-001); independently of that grant, `anon` cannot reach the delegate because it has no USAGE on schema `app` and no `EXECUTE` on `app.*` (the `SECURITY INVOKER` wrapper fails at the inner call).
...
- **Decision:** **DECISION D-036** —
  ...
  - **Grant posture mirrors the app RPC exactly:** `revoke all ... from public` + `grant execute ... to authenticated` — never `anon`/`service_role` (**D-011**); **anon cannot write** — the `anon` revoke must be explicit (hosted default privileges stamp a separate `anon` grant that a PUBLIC revoke does not remove; corrected by **D-037** / STOREFRONT-SEC-001), and independently `anon` has no USAGE on schema `app` and no `EXECUTE` on `app.sync_push`, so the `SECURITY INVOKER` wrapper fails at the inner call.
...
## D-037 — STOREFRONT-SEC-001 public-surface ACL posture: explicit `anon` allowlist

- **Status:** **PROPOSED** under the architecture-change procedure ([AGENT_WORKFLOW.md](AGENT_WORKFLOW.md) §9) — drafted with the SEC-001 migration (`supabase/migrations/20260913164029_public_surface_acl_remediation_001.sql`) and pinned by `supabase/tests/public_surface_acl_remediation_001_test.sql` (T-016). **NOT yet ratified:** pending independent Codex review and **human approval by Saleh at the merge gate**; the **hosted apply is a separately approved step** (pre-apply report: `docs/handoffs/BIZBOT_STOREFRONT_SEC_001_PREAPPLY_REPORT.md`). Takes the next free sequential ID **D-037**; **candidate D-030** stays reserved/unratified.
- **Context:** hosted Supabase carries a legacy schema-`public` default privilege (`ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public GRANT … TO anon`) that stamps every migration-created function, table, view and sequence with a separate explicit grant to the Postgres role `anon`; the repository's pre-2026-07-11 habit of `REVOKE … FROM PUBLIC` does not remove it, and the local Docker stack never carried it. A 2026-09-13 read-only inventory of production (project `oqmevrndtivqxgyvcmwy`) found **78 of 118** `public` functions and **51 of 55** `public` relations reachable by `anon` at the grant layer, held back only by forced RLS and by `anon` lacking USAGE on schema `app`. The Storefront (`menu.bizbot.systems`) will be the first product surface to use the publishable key from a public origin and the first to add anon-callable RPCs, so the incidental grants must be gone first and the rule must be explicit.
- **Decision:**
  1. The set of `public` functions EXECUTE-able by the Postgres role `anon` is an **explicit, enumerated allowlist**. Until the first Storefront public-read ticket ships it is **EMPTY** (Phase 1A posture, established by SEC-001).
  2. Every later anon-callable RPC joins the allowlist **by name and exact signature in its own migration**, with its own pgTAP proof executed as the **real** role `anon` (`set local role anon`, never faked JWT claims), an isolation review (**RISK R-003**) and an [API_CONTRACT](API_CONTRACT.md) entry. Nothing joins the allowlist incidentally.
  3. `anon` holds **no privilege** on any `public` table, view or sequence (RLS is a second barrier, never the only one) and **never** receives USAGE on schema `app`; `anon` keeps USAGE on schema `public` (PostgREST requires it and the allowlist lives there).
  4. Every `public` function revokes **both** PUBLIC **and** `anon` explicitly at creation (the post-2026-07-11 house form), and the migration-running owner's default privileges in `public` no longer grant `anon` anything (SEC-001). Per-schema default privileges are *added on top of* the built-in default, so the built-in PUBLIC EXECUTE on a brand-new function is closed by that per-function revoke, which T-016 enforces.
  5. `authenticated` and `service_role` grants are **unchanged** by this decision. Supabase anonymous-sign-in principals (paired POS/KDS/Kiosk devices) are role `authenticated`, not `anon`.
  6. Future Storefront RPCs are granted to `anon` only; they are **not** granted to `authenticated` unless a proven requirement exists (the Storefront BFF calls with the anon key).
- **Correction to D-035 / D-036 wording:** those ADRs described anon as "doubly blocked (revoked from public …)"; on hosted the PUBLIC revoke never removed the explicit `anon` grant, so the only barrier was the `app`-schema boundary. Their mechanism text is amended in place; the decisions themselves stand.
- **Consequences:** (+) the publishable key is safe by construction of grants, not by RLS alone; (+) T-016 fails CI when any new `public` function forgets its revokes or any migration re-grants `anon`; (−) every public Storefront RPC needs an explicit grant line and its own anon test; (−) hosted default-privilege drift is possible (a platform upgrade may re-apply the legacy `postgres` default; the `supabase_admin` defaults cannot be altered by `postgres`), so the [DEPLOYMENT](DEPLOYMENT.md) §16 PRE/POST queries are re-run after every platform upgrade and before every anon-allowlist ticket.
- **Related:** [SECURITY_AND_THREAT_MODEL](SECURITY_AND_THREAT_MODEL.md) (§3 Layer 3 requirement; §14 T-016), [API_CONTRACT](API_CONTRACT.md) (§4 preamble), [TESTING_STRATEGY](TESTING_STRATEGY.md) (§2, §6, §7), [DEPLOYMENT](DEPLOYMENT.md) (§16), D-011, D-012, D-029, D-035, D-036, **RISK R-003**.
...
- **Storefront-track (STOREFRONT-SEC-001): PROPOSED D-037** (explicit `anon` allowlist for `public` functions — **EMPTY** until the first Storefront public-read ticket; no `anon` privilege on `public` tables/views/sequences; `anon` never gets USAGE on `app`; every `public` function revokes PUBLIC **and** `anon` explicitly; the migration-owner default privileges in `public` no longer grant `anon`; corrects the "doubly blocked" mechanism text in D-035/D-036; hosted apply separately approved). The range becomes **D-001..D-029, D-031..D-037** (**D-030** reserved/unratified); the next free ID is **D-038+**.
