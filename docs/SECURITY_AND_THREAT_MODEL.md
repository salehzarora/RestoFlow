...
**SECURITY REQUIREMENT — public-schema function ACL / `anon` allowlist (DECISION D-037, STOREFRONT-SEC-001).** The Postgres role `anon` (a request carrying only the publishable key, no session) may EXECUTE **only** the functions on an explicit, enumerated allowlist — **empty** until the first Storefront public-read ticket ships — and holds **no** privilege on any `public` table, view or sequence and **never** USAGE on schema `app`. Every `public.*` function revokes **PUBLIC and `anon` explicitly** at creation (hosted Supabase's `ALTER DEFAULT PRIVILEGES` otherwise stamps a separate `anon` grant that a PUBLIC revoke does not remove), and the migration-owner default privileges in `public` grant `anon` nothing. Each future anon-callable RPC is reviewed individually, granted by exact signature in its own migration, and proven as the **real** role `anon` in pgTAP (T-016). Supabase anonymous-sign-in principals are role `authenticated`, not `anon`.
...
- **T-016 — The `anon` public surface equals the explicit allowlist.** Evaluated as the **real** database role `anon` (`set local role anon`, no faked JWT): the set of `public` functions with effective EXECUTE for `anon` equals the enumerated allowlist (**empty** at Phase 1A — **DECISION D-037**); `anon` has no USAGE on schema `app`, no privilege on any `public` table/view/sequence, and the migration-owner default privileges in `public` grant `anon` nothing; `authenticated` keeps EXECUTE on every `public` wrapper; no `public` function is `SECURITY DEFINER`. Pinned by `supabase/tests/public_surface_acl_remediation_001_test.sql`; the same catalog queries are the hosted PRE/POST check in [DEPLOYMENT](DEPLOYMENT.md) §16. *(D-011, D-012, D-037, RISK R-003)*
