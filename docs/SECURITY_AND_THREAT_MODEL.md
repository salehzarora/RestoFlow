...
- `cashier` — limited to assigned branch; may not void a paid order without an explicit permission grant (Section 14, T-006). Performs cash-drawer/shift close and count (`close_shift`); does **not** reconcile.
...
| TH-2 | **E**levation of Privilege — privilege escalation | Cashier/kitchen acts beyond scope, self-grants role, accountant attempts a mutation, or a tenant membership reaches for platform-admin | Membership-scoped roles (no global role); RPC re-authorizes; accountant strictly read-only; platform-admin is a separate platform-scoped grant (not a membership); denied attempts audited; DB constraints | D-004, D-005, D-012, D-013, D-026, D-028; Tests T-002, T-006, T-008, T-011 |
...
| TH-3 | **S**poofing / Tampering — stolen device | Lost/stolen paired device used to act or exfiltrate local DB | Device identity + revocation; encrypted local store + secure storage; bounded offline window; PIN re-auth + lockout | D-006, Q-009, R-007; Tests T-004 |
...
| TH-5 | **E**levation / Spoofing — offline-revoked actor | Revoked employee/device keeps acting offline | Future-access removal; server rejection on reconnect with audit; short offline validity | D-006, Q-009, R-007; Tests T-004, T-005 |
| TH-6 | **I**nformation Disclosure — secret leakage | Service-role key/secret in client or repo | No service-role key in clients; no secrets in repo; CI secret scanning; log redaction; TLS | D-011; Section 12 |
...
- **T-006 — Cashier cannot void a paid order without permission.** A `cashier` lacking the void permission **cannot** void a paid/submitted order; the void RPC denies the request, no payment/order state changes, and the denied attempt is audited. A void by an authorized actor requires a reason and is audited. *(TH-2, D-012/D-013, state model D-018)*
