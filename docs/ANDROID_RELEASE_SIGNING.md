...
| **0.0.53 / 53** | **`restoflow-production`** | main after the BIZBOT-LAUNCHER-TRANSPARENT PR (recorded at build time) | **planned — transparent launcher icons (nothing behind the symbol) for all four apps + the frameless POS lockup; kiosk 0.2.23 / 25 and dashboard 0.0.3 / 3 from the same source. NOT built yet** |
