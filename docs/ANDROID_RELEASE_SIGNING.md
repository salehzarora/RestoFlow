...
| **0.0.52 / 52** | **`restoflow-production`** | `4a3946b` (app source `8d6bc34a`) | **built 2026-09-06 (BIZBOT-ANDROID-RELEASE-V52) — the first pair carrying the final approved BIZBOT symbol + the POS navbar lockup; verified, NOT installed, NOT uploaded; kiosk 0.2.22 / 24 and the first production-signed dashboard 0.0.2 / 2 built from the same source. Promotion follows device validation** |
