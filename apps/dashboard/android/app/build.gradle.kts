...
// ---------------------------------------------------------------------------
// BIZBOT-ANDROID-RELEASE-V52 — FAIL-CLOSED production signing for the
// Dashboard Android package (RELEASE-KEY-AND-PIPELINE-001 pattern, ported
// verbatim from apps/kiosk, which ported it verbatim from apps/pos).
//
// Until this change the release buildType carried the stock Flutter template
// line `signingConfig = signingConfigs.getByName("debug")`. Every Dashboard
// APK so far was a pilot/debug build (0.0.1 / 1) that was never distributed;
// a "release" APK built that way is signed with the shared Android DEBUG key:
// not an identity anyone controls, not one that can own an app on a device
// long-term, and one that any machine can reproduce. Unacceptable for an
// official artifact.
//
...
//
// This block is duplicated verbatim in apps/pos, apps/kds and apps/kiosk. A
// shared script plugin was considered and rejected there: configuring the AGP
// `android` extension from an external .gradle.kts requires internal AGP types
// (no type-safe accessors outside the module), which is more fragile than a
// reviewed copy. The POS/KDS pair is kept honest by
// tools/android_release/check_signing_identity.ps1; the kiosk and dashboard
// copies are proven at release time by verifying the APK signer fingerprint
// against the same production certificate (verify_official_apk.ps1).
// ---------------------------------------------------------------------------
...
android {
    namespace = "com.restoflow.dashboard"
    ...
    defaultConfig {
        // RestoFlow Dashboard — pilot Android package (ANDROID-001B).
        applicationId = "com.restoflow.dashboard"
