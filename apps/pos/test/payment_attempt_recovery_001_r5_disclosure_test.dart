// PAYMENT-ATTEMPT-RECOVERY-001 / S1-R5 — the DUAL-DISCLOSURE regressions
// (F004), driven through the real payment sheet.
//
// Adopted UNCHANGED in substance from Codex's retained harness
// `reviewer_s1_r4_f004_actor_scope_variation_test.dart` (SHA-256
// 7F225B9E7E5F8D5D37C4EA1EF59605E33192AD845F7610DFFC95CB4BA8CF0772). Its 22
// cases are the 21 R4 regressions this file supersedes plus
// REVIEWER-S1R4-F004-ACTOR; that case ran 0/1 at 8d6b804f, twice.
//
// Every case asserts the REQUIRED SAFE outcome, so none needed a twin. Not one
// assertion was altered; only this header and the per-test register resets.
//
// The added case recreates the provider and host for a SECOND employee in the
// same authorized physical scope, after the first employee observed a remote
// refusal plus a failed local save. The unverified durable key still fails
// closed for everyone, but one cashier's exact refusal is never the other's.
//
// The sheet is driven through its real host and provider route, with a
// recording drawer service and a counting order-detail repository. No printer
// or drawer transport is touched.
import 'dart:convert';
...
void main() {
  ...
  testWidgets(
    'CODEX-F004-DISCLOSURE-001 second action in the same sheet keeps both truths',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
  ...
  testWidgets(
    'CODEX-F004-DISCLOSURE-002 sheet recreation must retain exact refusal and failed-save truth',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
      ...
      expect(
        find.byKey(const Key('payment-no-shift-banner')),
        findsOneWidget,
        reason: 'the exact memoized refusal remains true after sheet rebuild',
      );
      expect(
        find.byKey(const Key('payment-refusal-save-blocked-banner')),
        findsOneWidget,
        reason: 'the failed local save remains separately disclosed',
      );
      expect(server.paymentOpsSeen, hasLength(1));
      expect(server.completedPaymentsFor('order-1'), 0);
    },
  ...
  testWidgets(
    'CODEX-F004-DISCLOSURE-002b sheet recreation must retain failed-save disclosure',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
      ...
      expect(
        find.byKey(const Key('payment-refusal-save-blocked-banner')),
        findsOneWidget,
        reason: 'the failed local save remains visible after sheet rebuild',
      );
      expect(server.paymentOpsSeen, hasLength(1));
      expect(server.completedPaymentsFor('order-1'), 0);
    },
  ...
  testWidgets(
    'CODEX-F004-DISCLOSURE-003 provider-host rebuild must retain both truths',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
      final server = _server(shifts: const <String>{});
      final firstWorld = _world(server: server, prefs: failAfter);
      await _pumpHost(tester, firstWorld);
      await _open(tester);
      await _confirmCash(tester);
      ...
      Navigator.of(tester.element(find.byType(CashPaymentSheet))).pop();
      await tester.pumpAndSettle();
      final rebuiltWorld = _world(server: server, prefs: failAfter);
      await _pumpHost(tester, rebuiltWorld);
      await _open(tester);

      expect(
        find.byKey(const Key('payment-no-shift-banner')),
        findsOneWidget,
        reason: 'provider replacement must not erase exact remote truth',
      );
      expect(
        find.byKey(const Key('payment-refusal-save-blocked-banner')),
        findsOneWidget,
        reason: 'provider replacement must not erase degraded-storage truth',
      );
      expect(server.paymentOpsSeen, hasLength(1));
      expect(server.completedPaymentsFor('order-1'), 0);
    },
  ...
  testWidgets(
    'CODEX-F004-DISCLOSURE-003b provider-host rebuild must retain failed-save disclosure',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
      ...
      expect(
        find.byKey(const Key('payment-refusal-save-blocked-banner')),
        findsOneWidget,
        reason: 'failed local save remains visible after provider rebuild',
      );
      expect(server.paymentOpsSeen, hasLength(1));
      expect(server.completedPaymentsFor('order-1'), 0);
    },
  ...
  testWidgets(
    'REVIEWER-S1R4-F004-ACTOR disclosure is not disclosed to a different actor in the same physical scope',
    (tester) async {
      _size(tester);
      final prefs = await _freshPrefs();
      final failAfter = _FailAfterPrefs(prefs, 1);
      final server = _server(shifts: const <String>{});
      final firstActor = _world(
        server: server,
        prefs: failAfter,
        employee: 'emp-1',
      );
      await _pumpHost(tester, firstActor);
      await _open(tester);
      await _confirmCash(tester);
      ...
      final secondActor = _world(
        server: server,
        prefs: failAfter,
        employee: 'emp-2',
      );
      await _pumpHost(tester, secondActor);
      await _open(tester);

      expect(
        find.byKey(const Key('payment-quarantined-banner')),
        findsOneWidget,
        reason: 'the unverified durable key still fails closed for every actor',
      );
      expect(
        find.byKey(const Key('payment-no-shift-banner')),
        findsNothing,
        reason: 'an exact refusal observed by emp-1 is not emp-2 disclosure',
      );
      expect(
        find.byKey(const Key('payment-refusal-save-blocked-banner')),
        findsNothing,
        reason: 'the failed-save register must be actor-scoped',
      );
      expect(server.paymentOpsSeen, hasLength(1));
      expect(server.completedPaymentsFor('order-1'), 0);
      expect(secondActor.drawer.kicks, isEmpty);
      expect(secondActor.detail.fetches, 0);
    },
  );
}
