...
import 'table_picker_sheet.dart' show TableGroupCardData, localizedTableArea;
...
class TableGroupDetailSheet extends ConsumerStatefulWidget {
  ...
  static Future<void> show(
    ...
    showDragHandle: true,
    builder: (_) =>
        TableGroupDetailSheet(group: group, onSelectMember: onSelectMember),
  );
...
class _TableGroupDetailSheetState extends ConsumerState<TableGroupDetailSheet> {
  ...
  Future<void> _unlink() async {
    ...
    try {
      ...
      // member dissolves it for all (orders untouched). No new write path.
      await ref
          .read(tableOperationsRepositoryProvider)
          .unlink(tableId: _group.members.first.tableId);
  ...
  Widget build(BuildContext context) {
    ...
    final g = _group;
    final anyAssignable = g.members.any((m) => m.isAssignable);
    final canManage =
        ref
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '${l10n.posTableGroupDetailTitle} · ${g.combinedLabel(l10n.posTableGroupJoiner)}',
            ...
            _kv(
              ...
              _stateLabel(l10n, g.effectiveState),
              key: const Key('group-detail-effective'),
            ),
            _kv(
              theme,
              l10n.posTableActiveOrders,
              l10n.posTableOpenOrders(g.activeOrderCount),
                ...
                children: [
                  for (final m in g.members)
                    ListTile(
                      ...
                      trailing: m.isAssignable
                          ? FilledButton.tonal(
                              key: Key('group-member-select-${m.tableId}'),
                              onPressed: _submitting ? null : () => _select(m),
