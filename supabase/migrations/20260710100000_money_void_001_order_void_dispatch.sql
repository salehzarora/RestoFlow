-- ============================================================================
-- MONEY-VOID-001 - Make role-gated order VOID reachable from the POS outbox
-- ============================================================================
-- The audit (RESTOFLOW-AUDIT-001) found a wrong/duplicate order has no operator
-- recourse: app.void_order (RF-053, hardened by RF-062) exists and is correct,
-- but is UNREACHABLE - app.sync_push never dispatched an 'order.void' op and the
-- sync_operations.operation_type CHECK did not allow it. REACHABILITY-only fix;
-- reimplements NO business logic.
--
-- WHAT THIS DOES (forward-only, additive):
--   1. Extends sync_operations.operation_type CHECK to accept 'order.void'.
--   2. CREATE OR REPLACE app.sync_push - a FAITHFUL re-creation of the newest
--      body (20260707120000_order_customer_001_customer_name.sql) with EXACTLY
--      three additions: 'order.void' in BOTH envelope allowlists and a new
--      'order.void' dispatch branch calling app.void_order with the SAME pin
--      session + device + the op local_operation_id. submit_order, the
--      ORDER-CUSTOMER-001 customer_name stamp, and every other branch are
--      byte-unchanged (do NOT start from an older sync_push - regresses
--      customer_name).
--
-- NOT CHANGED: app.void_order itself (already enforces role gating, mandatory
--   reason, tenant scope, legal source states, the completed-payment block, and
--   idempotency); order_operations.action CHECK already permits 'void_order'
--   (rf054); grants (sync_push is SECURITY DEFINER, so its internal void_order
--   call runs as the function owner); reports (owner_daily_report /
--   owner_report_range / sales_summary already EXCLUDE 'voided' from billed +
--   collected and surface it in the voids KPI); order history (already returns +
--   labels 'voided'); KDS (already drops 'voided' via _activeOrderStatuses -
--   money-free). NO payment/tax/total logic changes; NO data deletion.
--
-- MONEY SAFETY: a void creates/deletes NO payment and recomputes NO total; it
--   only sets orders.status='voided' + void_reason + revision (integer minor
--   money, D-007, is untouched).
--
-- FORWARD-ONLY (Supabase replays on db reset). Manual teardown at the foot.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. sync_operations.operation_type CHECK - extend with 'order.void'
--    (mirrors the mvp_order_status_sync order.status extension).
-- ----------------------------------------------------------------------------
...
alter table public.sync_operations add constraint sync_operations_operation_type_check
  check (operation_type in ('shift.open', 'order.submit', 'order.discount', 'payment.create', 'shift.close', 'order.status', 'order.void'));

-- ----------------------------------------------------------------------------
-- 2. app.sync_push - CREATE OR REPLACE (newest body + the order.void branch).
-- ----------------------------------------------------------------------------
...
begin
  ...
  if not (v_ds_active and v_ds_revoked is null and v_pairing = 'active') then
    ...
    loop
      ...
      if v_op_type is null or v_op_type not in ('shift.open', 'order.submit', 'order.discount', 'payment.create', 'shift.close', 'order.status', 'order.void') then
...
-- ----------------------------------------------------------------------------
-- MANUAL TEARDOWN (forward-only; not auto-run). To reverse locally:
--   alter table public.sync_operations drop constraint if exists sync_operations_operation_type_check;
--   alter table public.sync_operations add constraint sync_operations_operation_type_check
--     check (operation_type in ('shift.open', 'order.submit', 'order.discount', 'payment.create', 'shift.close', 'order.status'));
--   -- then re-create app.sync_push from 20260707120000_order_customer_001_customer_name.sql
-- ----------------------------------------------------------------------------
