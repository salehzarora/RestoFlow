-- ============================================================================
-- KITCHEN-DISPATCH-ENFORCE-001 — SERVER-SIDE dispatch-mode enforcement.
--
-- KITCHEN-PRINT-DUAL-001C shipped the direct_print dispatch as a CLIENT-ONLY
-- decision: app.sync_push routed the order.submit arm purely on
-- payload->>'dispatch_mode' and never compared it against the authoritative
-- branches.kitchen_workflow_mode. A forged, malformed, stale or replayed
-- direct_print submit on a KDS branch therefore stamped the order `served`,
-- dropped it out of the KDS feed and created NO kitchen print dispatch — the
-- ticket reached neither the screen nor the printer.
--
-- This migration closes that gap with defence in depth (D-012), forward-only
-- and additive (no table DDL, no new function, no new grant surface):
--
--   1. app.sync_push — byte-faithful re-emit of the 20260802090000 body, with
--      ONLY: two new declares, an INGEST-TIME validation block in the
--      order.submit arm (positioned AFTER the customer-phone guard and BEFORE
--      app.submit_order, so a rejection creates NO business rows), the routing
--      condition switched to the validated variable, and normalization of the
--      belt's dedicated SQLSTATE in the existing per-op EXCEPTION handler.
--
--      The authoritative branch read for the PRIVILEGED direct_print arm takes
--      a FOR SHARE row lock held to transaction end. FOR SHARE conflicts with
--      the FOR NO KEY UPDATE lock an `update branches set
--      kitchen_workflow_mode = ...` takes, so acceptance is SERIALIZED against
--      mode changes: either the submit commits under the mode authoritative
--      for its transaction (and the update waits), or the update commits first
--      and the submit waits, re-reads `kds` and rejects. There is NO outcome
--      in which a direct_print op commits as newly ACCEPTED on a stale
--      printer_only read. Concurrent direct_print submits SHARE the lock. The
--      common absent/'kds' path performs no branches read here and takes NO
--      lock at all.
--
--   2. app.apply_direct_print_dispatch — byte-faithful re-emit of the
--      20260729090000 body plus a defensive belt: it re-proves the branch is
--      affirmatively printer_only before the privileged promotion and
--      otherwise raises the DEDICATED internal SQLSTATE RFDM0, which
--      app.sync_push maps to the terminal typed public error
--      `dispatch_mode_not_allowed`. No raw SQLSTATE and no internal
--      branch-mode detail ever reaches a client.
--
-- Approved compatibility matrix (deployed-client safe):
--   kds          + absent / 'kds'  -> accept      printer_only + 'direct_print' -> accept
--   kds          + 'direct_print'  -> REJECT      printer_only + absent/'kds'   -> accept
...
-- and every KDS lifecycle state are untouched. No state is added or renamed.
-- ============================================================================


-- ----------------------------------------------------------------------------
-- app.sync_push — re-emit with the ingest-time dispatch-mode guard.
-- ----------------------------------------------------------------------------
  ...
  loop
    ...
    begin
      case v_op_type
        ...
        when 'order.submit' then
          ...
          -- precedent). Anything but an affirmative 'printer_only' — including a
          -- missing or tombstoned branch row — fails CLOSED.
    ...
    if v_caught_state is not null then
      if v_caught_state = '40001' then
        ...
        -- SAME terminal typed rejection the primary ingest guard returns — the
...
comment on function app.sync_push(uuid, uuid, jsonb) is
  'RF-056/RF-061 + ... + PSC-001D + PSC-001C (D-010/D-022) SECURITY DEFINER batch push — faithful re-creation of the 20260722090000 body; all 15 canonical operations, identity hardening, atomic ledger claim, batch cap, result ordering, dependency guard, per-op subtransactions, finalization and the customer_name stamp are verbatim. KITCHEN-MODE-001C1-CORRECTION-001 (order.submit only): immediately after the customer_name stamp, the order''s still-unclaimed initial kitchen dispatch payload is REBUILT through app.kitchen_dispatch_payload_initial in the SAME transaction, so the REAL push path carries customer_display_name exactly like the direct-call path; kds branches are a structural no-op and claimed/completed/superseded dispatches are never touched. Authorization INGEST-TIME; scope from the session, never the payload. KITCHEN-DISPATCH-ENFORCE-001 (order.submit only): the REQUESTED dispatch_mode is validated at ingest BEFORE app.submit_order runs, so a rejection creates NO business rows — an unknown value is invalid_payload/dispatch_mode, and ''direct_print'' requires an affirmative branches.kitchen_workflow_mode = ''printer_only'' read FOR SHARE from the SESSION-derived scope (fail closed on a missing/tombstoned row), which serializes acceptance against concurrent kitchen_workflow_mode updates; absent/''kds'' submissions take NO lock and are accepted on every branch mode (deployed-client contract). The app.apply_direct_print_dispatch belt''s dedicated SQLSTATE is normalized here to the terminal typed dispatch_mode_not_allowed — no raw SQLSTATE or branch-mode detail ever reaches a client.';
...
-- ----------------------------------------------------------------------------
-- app.apply_direct_print_dispatch — re-emit with the defensive branch-mode belt.
-- ----------------------------------------------------------------------------
...
begin
  ...
  -- KITCHEN-DISPATCH-ENFORCE-001 DEFENSIVE BELT (defence in depth, D-012).
  -- This promotion is privileged: it routes an order OUT of the KDS active
  -- workflow. It is legal ONLY on an affirmatively `printer_only` branch. On
  -- the app.sync_push path the ingest guard already share-locked this exact
  -- row in this same transaction, so this re-read never waits and the belt is
  -- unreachable by construction; it exists for any FUTURE caller and against
  -- code drift, and it is self-sufficiently serialized by the same FOR SHARE.
  -- The raise carries the DEDICATED internal code RFDM0 and NO branch-mode or
  -- internal detail in its message; app.sync_push normalizes it to the
  -- terminal typed public error `dispatch_mode_not_allowed`. Raising rolls the
  -- caller's per-op subtransaction back, so no business rows survive.
  select b.kitchen_workflow_mode
    into v_branch_mode
    from public.branches b
    where b.id              = p_branch_id
      ...
      and b.deleted_at is null
    for share;
  if coalesce(v_branch_mode, '') <> 'printer_only' then
    raise exception 'direct print dispatch is not permitted for this branch'
      using errcode = 'RFDM0';
  end if;
