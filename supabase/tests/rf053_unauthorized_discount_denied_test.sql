-- ============================================================================
-- RF-053 — pgTAP: unauthorized discount is denied + audited (A6, D-013/§116)
-- ============================================================================
-- A plain cashier (no apply_discount permission) cannot apply a discount: the RPC
-- returns permission_denied, makes NO state change, and writes an
-- order.discount_denied audit row (mirroring void_order's denied path). A cashier
-- with the explicit permissions.apply_discount grant succeeds. Fixtures inserted
...
-- plain cashier (no apply_discount grant) => denied + audited, no change ------ 1-3
select is(
  ...
  'permission_denied', 'a plain cashier (no apply_discount grant) is denied');
...
-- a cashier WITH the explicit grant succeeds -------------------------------- 4-5
select is(
  ...
  true, 'a cashier with permissions.apply_discount=true may apply a discount');
