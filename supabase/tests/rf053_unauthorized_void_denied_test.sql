-- ============================================================================
-- RF-053 — pgTAP: an EXPLICITLY-DENIED cashier's void is denied + audited
--                 (AC#1, T-006)
-- ============================================================================
-- STAFF-CASHIER-PERMISSIONS-001: cashier UNPAID-order void is now default-ON, so
-- an UNAUTHORIZED cashier is one carrying the explicit deny override
-- permissions.void_order='false'. Such a cashier cannot void a submitted order:
-- the RPC returns permission_denied, makes NO state change, and writes an
-- order.void_denied audit row. A manager can void. (A plain default cashier is now
-- allowed to void an unpaid order — covered by
-- staff_cashier_permissions_001_test.sql; the T-006 PAID-void protection is
-- unchanged.) Fixtures inserted as the BYPASSRLS role; the RPCs are DEFINER.
-- ============================================================================
begin;
...
-- explicit-deny cashier void => permission_denied, no state change, audited -- 1-4
select is(
