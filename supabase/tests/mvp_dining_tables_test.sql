...
-- (nullable; legacy rows carry nulls). TABLE-VISUAL-LAYOUT-118 added two
-- presentation keys: visual_preset + section_floor_preset (nullable; NULL =
-- the default look). STALE-TABLE-ORDER-RECOVERY-001 added active_orders.
-- Eighteen keys now (the exact key SET is pinned by
-- table_floor_layout_021_test / table_visual_presets_118_test).
select is(
  ...
  18, 'a pos_tables row carries the eighteen keys (eight prior + five 021 layout keys + two 118 preset keys + the 120 visual_material + the 121 section_room_frame_preset + the STALE-TABLE-ORDER-RECOVERY-001 active_orders)');
select ok(
