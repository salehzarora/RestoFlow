...
-- ---- sellable menu fixture (submit_order requires proven-sellable items) ---
insert into menu_categories (id, organization_id, restaurant_id, branch_id, name, display_order) values
  ('00000000-0000-0000-0000-0000000ca7a1', '00000000-0000-0000-0000-0000000000a0', '00000000-0000-0000-0000-0000000000a1', null, 'Fixture Food', 1);
...
-- exact expectations, but append-only residue from the concurrency harness
-- in the same disposable db must not be absorbed by whole-table reads.)
select is((select count(*) from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0')::int, 1, 'submit_order wrote exactly one audit_events row');
select is((select action from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0')::text, 'order.submitted', 'audit action is order.submitted');

-- tenant context + device + actor -------------------------------------------- 3-7
select is((select organization_id from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-0000000000a0'::uuid, 'audit organization_id = resolved org');
select is((select restaurant_id  from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-0000000000a1'::uuid, 'audit restaurant_id = resolved restaurant');
select is((select branch_id      from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-00000000a1b1'::uuid, 'audit branch_id = resolved branch');
select is((select device_id      from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-00000000da11'::uuid, 'audit device_id = p_device_id');
select is((select actor_employee_profile_id from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-0000000ef001'::uuid, 'audit actor_employee_profile_id = resolved employee (RF-017 actor present)');
...
select ok(
  (select new_values ?& array[
   ...
   from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'),
  'new_values contains all 16 expected keys');

-- and each remaining key carries the correct value --------------------------- 15-24
select is((select new_values ->> 'status'                      from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 'submitted',  'new_values.status');
select is((select (new_values ->> 'discount_total_minor')::bigint from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 0::bigint, 'new_values.discount_total_minor');
select is((select (new_values ->> 'tax_total_minor')::bigint    from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 0::bigint,    'new_values.tax_total_minor');
select is((select new_values ->> 'device_id'                   from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-00000000da11', 'new_values.device_id');
select is((select new_values ->> 'order_type'                  from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 'dine_in',    'new_values.order_type');
select is((select new_values ->> 'table_id'                    from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-0000000ab1e1', 'new_values.table_id');
select is((select new_values ->> 'shift_id'                    from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-0000000585f1', 'new_values.shift_id');
select is((select new_values ->> 'resolved_membership_id'      from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), '00000000-0000-0000-0000-00000000ab01', 'new_values.resolved_membership_id');
select is((select (new_values ->> 'item_count')::int           from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 1, 'new_values.item_count');
select is((select (new_values ->> 'modifier_count')::int       from audit_events where organization_id = '00000000-0000-0000-0000-0000000000a0'), 1, 'new_values.modifier_count');
