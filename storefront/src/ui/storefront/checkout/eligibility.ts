 ...
 * readiness first (nothing to decide against yet), then the restaurant's
...
export type OrderingBlocker = 'unresolved' | 'closed' | 'paused';
 ...
 * `null` is "not resolved"; any value other than the three known ones is
 * treated as unresolved too - never as open.
 */
export function orderingBlocker(state: ServiceState | null): OrderingBlocker | null {
...
export function submitEligibility(input: {
  ...
  const ordering = orderingBlocker(input.state);
...
/**
 * The localised reason a step shows while ordering is not open: the same two
 * strings the cart and the dock already use (Storefront.dc.html:398,
 * CONTENT_AND_LOCALIZATION.md). Nothing is invented for the unresolved case:
 * a step that cannot decide yet shows its pending frame, not a sentence.
 */
